...
export default function PROJECTCLIENTVIEW2({
  onClose,
  project,
}: {
  onClose: () => void;
  ...
  const handleApply = async (projectId: number) => {
    console.log(projectId, "project id selected");
    ...
    try {
      const projectApplicationCommand: ApplyToProjectRequest = {
        projectId,
        techStackRequirementIds: selectedStacks,
      };
  ...
  const allSkillsSelected = project.projectTechStacks.every((tech) =>
    selectedStacks.includes(tech?.techStack.id ?? 0)
  );
  const anySkillsSelected = selectedStacks.length > 0;

  const buttonText = allSkillsSelected
    ? "I'll take this solo"
    : anySkillsSelected
    ? "Wait for a teammate"
                ...
                {project.projectTechStacks.map((projectTechStack) => {
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ y: 3 }}
                      className={` ${baseStyle} ${opacityStyle} text-sm px-3 py-1 rounded-md`}
                    >
